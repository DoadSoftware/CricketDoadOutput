...
public class BugsAndMiniGfx 
{
	...
	public BugsAndMiniGfx(List<PrintWriter> print_writers, Configuration config, List<Bugs> bugs, List<PerformanceBug> performanceBugs, List<Team> teams, List<VariousText> VariousText,CricketService cricketService,List<HeadToHead> headToHead, List<MatchAllData> tournament_matches) {
		super();
	...
	public String PopulateBugBody(int WhichSide, String whatToProcess,MatchAllData matchAllData) {
		...
		case Constants.NPL:
			...
			case "Control_y":
				...
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Bugs_All$MainTxt_Grp$Side" + WhichSide 
						+ "$txt_Balls*GEOM*TEXT SET POWERPLAY\0", print_writers);
				
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Bugs_All$MainTxt_Grp$Side" + WhichSide 
						+ "$txt_Name*GEOM*TEXT SET " + matchAllData.getMatch().getInning().get(inning.getInningNumber() - 1).getBatting_team().getTeamName1().toUpperCase() + "\0", print_writers);
				...
				for (Inning inn : matchAllData.getMatch().getInning()) {
					...
					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Bugs_All$SubText$Side" + WhichSide 
							+ "$txt_Sub*GEOM*TEXT SET " + CricketFunctions.getPowerPlayScore(inn, inning.getInningNumber(), "-", matchAllData) + "\0", print_writers);
			...
			case "Control_k":
				...
				for(BattingCard bc : inning.getBattingCard()) {
					if(bc.getPlayerId() == partnership.getFirstBatterNo()) {
						PartName1 = bc.getPlayer().getTicker_name();
					}else if(bc.getPlayerId() == partnership.getSecondBatterNo()) {
						PartName2 = bc.getPlayer().getTicker_name();
					}
				}
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Bugs_All$MainTxt_Grp$Side" + WhichSide 
						+ "$txt_Name*GEOM*TEXT SET " + PartName1 + " & " + PartName2 + "\0", print_writers);
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Bugs_All$MainTxt_Grp$Side" + WhichSide 
						+ "$txt_Runs*GEOM*TEXT SET \0", print_writers);
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Bugs_All$MainTxt_Grp$Side" + WhichSide 
						+ "$txt_Balls*GEOM*TEXT SET CURRENT PARTNERSHIP\0", print_writers);
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Bugs_All$SubText$Side" + WhichSide 
			...
			case "Shift_F4":
				...
				for(BattingCard bc : inning.getBattingCard()) {
					if(bc.getPlayerId() == partnership.getFirstBatterNo()) {
						Name1 = bc.getPlayer().getTicker_name();
					}else if(bc.getPlayerId() == partnership.getSecondBatterNo()) {
	...
	public String populateMiniBody(int WhichSide, String whatToProcess, MatchAllData matchAllData, int WhichInning) {
		...
		case Constants.NPL:
			...
			case "Shift_F1":
				...
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$minis$Side" + WhichSide 
						+ "$Batting$txt_FirstName*GEOM*TEXT SET " + inning.getBatting_team().getTeamName2() + " \0", print_writers);
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$minis$Side" + WhichSide 
						+ "*FUNCTION*Omo*vis_con SET 1 \0", print_writers);
			...
			case "Shift_F2":
				...
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$minis$Side" + WhichSide 
						+ "$Bowling$txt_FirstName*GEOM*TEXT SET " + inning.getBowling_team().getTeamName2() + " \0", print_writers);
