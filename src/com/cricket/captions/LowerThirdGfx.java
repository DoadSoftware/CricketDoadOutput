	...
	public String populateL3rdThisSeries(String whatToProcess,int WhichSide,MatchAllData matchAllData) throws InterruptedException, IOException
	{
		...
		if(whatToProcess.split(",")[0].equalsIgnoreCase("Control_s")) {
			...
			lowerThird = new LowerThird("", tournament.getPlayer().getFirstname(), surName,"ICC U19 CWC 2024", "", "", 2,"",teamName,
					...
					String.valueOf(tournament.getWickets()),Data,economy,best},null,null,new String[] {"-503.0","-315.0","-116.0","115.0","350.0","560.0"});
...
//			setPositionOfLT(lowerThird.getNumberOfSubLines(), WhichSide, 4,print_writers, config);
	...
	public String populateL3rdAllRounderStats(String whatToProcess, int WhichSide, MatchAllData matchAllData) throws InterruptedException {
		if (matchAllData == null || matchAllData.getMatch() == null || matchAllData.getMatch().getInning() == null) {
			...
			String runs="",overs="",balls="",strike="",wickets="",economy="",player="",team="",flag="",type="";int total_bl=0;
			if(whatToProcess.split(",")[2].equalsIgnoreCase("Economy")||whatToProcess.split(",")[2].equalsIgnoreCase("Catches")) {
				if( matchAllData.getMatch().getInning().get(0).getBowlingCard()
					...
					BowlingCard bow=matchAllData.getMatch().getInning().get(0).getBowlingCard()
							.stream().filter(boc->boc.getPlayerId()==Integer.valueOf(whatToProcess.split(",")[3]))
							.findAny().orElse(null);
					if(bow!=null) {
						player=bow.getPlayer().getFull_name();
						...
						runs=String.valueOf(bow.getRuns());
						balls=String.valueOf(Integer.valueOf((bow.getOvers()*6)+bow.getBalls()));
						if(bow.getWickets()==0) {
							strike="-";
							wickets="-";
						}else {
							wickets=String.valueOf(bow.getWickets());
							strike=String.valueOf(String.format("%.2f", Double.valueOf((bow.getBalls())+bow.getOvers())/Integer.valueOf(bow.getWickets())));
						}if(bow.getBalls()!=0)	{
							overs=String.valueOf(bow.getOvers()+"."+bow.getBalls());
						}else if(bow.getBalls()==0 &&bow.getOvers()!=0 ){
							overs=String.valueOf(bow.getOvers());
						}else if(bow.getBalls()==0 &&bow.getOvers()==0 ){
							overs="-";
						}
						if(whatToProcess.split(",")[2].equalsIgnoreCase("Economy")) {
							type="ECONOMY";
							economy=String.valueOf(bow.getEconomyRate());
						}else if(whatToProcess.split(",")[2].equalsIgnoreCase("Catches")) {
							type="CATCHES";
							economy=String.valueOf(CricketFunctions.getAllRounderCatches(Integer.valueOf(whatToProcess.split(",")[3]), matchAllData, matchAllData.getEventFile().getEvents()));
						}
					}
				}else if( matchAllData.getMatch().getInning().get(1).getBowlingCard()
						.stream().filter(boc->boc.getPlayerId()==Integer.valueOf(whatToProcess.split(",")[3]))
						.findAny().orElse(null) != null) {
					BowlingCard bow=matchAllData.getMatch().getInning().get(1).getBowlingCard()
							.stream().filter(boc->boc.getPlayerId()==Integer.valueOf(whatToProcess.split(",")[3]))
							.findAny().orElse(null);
					if(bow!=null) {
						player=bow.getPlayer().getFull_name();
				...
				if(statsType==null && stat.getPlayer_id()!=0) {
				...
				if(Integer.valueOf(stat.getStats_type_id())==Integer.valueOf(statsType.getStats_id())
					...
					if( matchAllData.getMatch().getInning().get(0).getBowlingCard()
						...
						if(bow!=null) {
							player=bow.getPlayer().getFull_name();
							total_bl=bow.getOvers()*6+bow.getBalls();
							team=matchAllData.getMatch().getInning().get(0).getBowling_team().getTeamName1();
							flag=matchAllData.getMatch().getInning().get(0).getBowling_team().getTeamName4();
							if(stat.getRuns_conceded()+bow.getRuns()==0) {
								runs="-";
							}else {
								runs=String.valueOf(stat.getRuns_conceded()+bow.getRuns());
							}if(stat.getWickets()+bow.getWickets()==0) {
								wickets="-";
								strike="-";
							}else {
								wickets=String.valueOf(stat.getWickets()+bow.getWickets());
								strike=String.valueOf(String.format("%.2f", Double.valueOf(((Integer.valueOf(stat.getRuns_conceded()+bow.getRuns()))/Integer.valueOf((stat.getWickets()+bow.getWickets()))))));	
							}	
						}
					}else if( matchAllData.getMatch().getInning().get(1).getBowlingCard()
							.stream().filter(boc->boc.getPlayerId()==Integer.valueOf(whatToProcess.split(",")[3]))
					...
					lowerThird = new LowerThird(player,team, "","", "","", 
							...
							new String[]{String.valueOf(stat.getMatches()+1),
									runs,strike,wickets,String.valueOf(CricketFunctions.getEconomy((Integer.valueOf(runs+stat.getRuns_conceded())), Integer.valueOf(stat.getBalls_bowled()+total_bl), 2, "-"))},null,
	...
	public String populateL3rdPowerPlay(String whatToProcess, int WhichSide, MatchAllData matchAllData) throws InterruptedException {
		...
		if (matchAllData == null || matchAllData.getMatch() == null || matchAllData.getMatch().getInning() == null) {
			...
			if(whatToProcess.split(",")[2].equalsIgnoreCase("p1")) {
				powerplay = "POWERPLAY 1";
				Subheader = "(OVERS " + inning.getFirstPowerplayStartOver() + " TO " + inning.getFirstPowerplayEndOver() + ")";
				pp_ovrs= CricketFunctions.PowerPlayMatchOvers(inning.getInningNumber(),matchAllData,",").split(",")[0];
				pp=CricketFunctions.getFirstPowerPlayScore(matchAllData,inning.getInningNumber(), matchAllData.getEventFile().getEvents());
				run_rate = CricketFunctions.generateRunRates(Integer.valueOf(pp.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[0].split("-")[1]),2,matchAllData);

			}else if(whatToProcess.split(",")[2].equalsIgnoreCase("p2")) {
				powerplay = "POWERPLAY 2";
				Subheader = "(OVERS " + inning.getSecondPowerplayStartOver() + " TO " + inning.getSecondPowerplayEndOver() + ")";
				pp_ovrs= CricketFunctions.PowerPlayMatchOvers(inning.getInningNumber(),matchAllData,",").split(",")[1];
				pp=CricketFunctions.getSecPowerPlayScore(matchAllData, inning.getInningNumber(), matchAllData.getEventFile().getEvents());
				run_rate = CricketFunctions.generateRunRates(Integer.valueOf(pp.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[1].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[1].split("-")[1]),2,matchAllData);

			}else if(whatToProcess.split(",")[2].equalsIgnoreCase("p3")) {
				powerplay = "POWERPLAY 3";
				Subheader = "(OVERS " + inning.getThirdPowerplayStartOver() + " TO " + inning.getThirdPowerplayEndOver() + ")";
				pp_ovrs= CricketFunctions.PowerPlayMatchOvers(inning.getInningNumber(),matchAllData,",").split(",")[2];
				pp=CricketFunctions.getThirdPowerPlayScore(matchAllData, inning.getInningNumber(), matchAllData.getEventFile().getEvents());
				run_rate = CricketFunctions.generateRunRates(Integer.valueOf(pp.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[2].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[2].split("-")[1]),2,matchAllData);
	...
	public String populateL3rdInningPowerPlay(String whatToProcess, int WhichSide, MatchAllData matchAllData) throws InterruptedException {
		...
		if (matchAllData == null || matchAllData.getMatch() == null || matchAllData.getMatch().getInning() == null) {
			...
			if(inning.getInningNumber()==1) {
				...
				run_rate = CricketFunctions.generateRunRates(Integer.valueOf(pp1.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[0].split("-")[1]),2,matchAllData)
						+","+CricketFunctions.generateRunRates(Integer.valueOf(pp2.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[1].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[1].split("-")[1]),2,matchAllData)+","
						+CricketFunctions.generateRunRates(Integer.valueOf(pp3.split(",")[0].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[2].split("-")[0]),Integer.valueOf(pp_ovrs.split(",")[2].split("-")[1]),2,matchAllData);
...
//			setPositionOfLT(lowerThird.getNumberOfSubLines(), WhichSide, 4,print_writers, config);
		...
		throws JsonMappingException, JsonProcessingException, InterruptedException
	{
		...
		if(whatToProcess.split(",")[0].equalsIgnoreCase("F7")) {
			...
			if(CricketFunctions.getAverage(stat.getMatches(), stat.getNot_out(), stat.getRuns(), 2, "-").equalsIgnoreCase("0.00")) {
				batAverage = "-";
			}else {
		...
		case Constants.ICC_U19_2023:
			...
			case "n":
				...
				for(int i=1; i<=4; i++) {
					...
					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$All_LowerThirds$Sublines$Side_" + WhichSide +
							"$Select_Subline$" + i + "$Divider$obj_Divider3*TRANSFORMATION*POSITION*X SET -233.0 \0",print_writers);
				}
				break;
